...
static input int    Magic_Number = 2062411610; // Magic number
static input double Entry_Amount =     1.00; // Entry lots
       input int    Stop_Loss    =       86; // Stop Loss   (pips)
       input int    Take_Profit  =        0; // Take Profit (pips)

static input string ___0______   = "------"; // --- Alligator ---
       input int    Ind0Param0   =       38; // Jaws period
       input int    Ind0Param1   =        6; // Jaws shift
       input int    Ind0Param2   =       16; // Teeth period
       input int    Ind0Param3   =        9; // Teeth shift
       input int    Ind0Param4   =        9; // Lips period
       input int    Ind0Param5   =        6; // Lips shift

static input string ___1______   = "------"; // --- Pin Bar ---
       input int    Ind1Param0   =        9; // Max body part %
       input int    Ind1Param1   =       26; // Min wick part %
...
void InitIndicators(void)
  {
   // Alligator (Smoothed, Median, 38, 6, 16, 9, 9, 6)
...
void ManageClose(void)
  {
...

   {
      ...
      bool isPinBar = closeOpen < Ind1Param0 * 0.01 * (High(1) - Low(1));
      bool upWick   = High(1) - MathMax(Close(1), Open(1)) > Ind1Param1 * 0.01 * closeOpen;
      bool downWick = MathMin(Close(1), Open(1)) - Low(1)  > Ind1Param1 * 0.01 * closeOpen;
...
/*STRATEGY CODE {"properties":{"entryLots":1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":86,"takeProfit":0,"useStopLoss":true,"useTakeProfit":false,"isTrailingStop":false},"openFilters":[{"name":"Alligator","listIndexes":[9,3,4,0,0],"numValues":[38,6,16,9,9,6]}],"closeFilters":[{"name":"Pin Bar","listIndexes":[0,0,0,0,0],"numValues":[9,26,0,0,0,0]}]} */
