...
static input int    Magic_Number = 424784384; // Magic number
...
static input string ___0______   = "------"; // --- Stochastic ---
       input int    Ind0Param0   =        2; // %K Period
       input int    Ind0Param1   =        2; // %D Period
       input int    Ind0Param2   =        1; // Slowing
       input double Ind0Param3   =     91.0; // Level

static input string ___1______   = "------"; // --- Moving Average ---
       input int    Ind1Param0   =       20; // Period
       input int    Ind1Param1   =        0; // Shift

static input string ___2______   = "------"; // --- Williams' Percent Range ---
       input int    Ind2Param0   =       30; // Period
       input double Ind2Param1   =    -20.0; // Level

static input string ___3______   = "------"; // --- Stochastic ---
       input int    Ind3Param0   =       16; // %K Period
       input int    Ind3Param1   =        2; // %D Period
       input int    Ind3Param2   =        9; // Slowing
       input double Ind3Param3   =     51.0; // Level
...
void InitIndicators(void)
  {
   // Stochastic (2, 2, 1), Level: 91.0
   indHandlers[0][0][0] = iStochastic(NULL, 0, Ind0Param0, Ind0Param1, Ind0Param2, MODE_SMA, 0);
   ...
   // Williams' Percent Range (30)
   indHandlers[0][2][0] = iWPR(NULL, 0, Ind2Param0);
   if(Show_inds) ChartIndicatorAdd(0, 2, indHandlers[0][2][0]);
   // Stochastic (16, 2, 9), Level: 51.0
   indHandlers[0][3][0] = iStochastic(NULL, 0, Ind3Param0, Ind3Param1, Ind3Param2, MODE_SMA, 0);
   if(Show_inds) ChartIndicatorAdd(0, 3, indHandlers[0][3][0]);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
int GetEntrySignal(void)
  {
   // Stochastic (2, 2, 1), Level: 91.0
   double ind0buffer[]; CopyBuffer(indHandlers[0][0][0], MAIN_LINE, 1, 3, ind0buffer);
   double ind0val1  = ind0buffer[2];
   bool   ind0long  = ind0val1 > Ind0Param3 + sigma;
   bool   ind0short = ind0val1 < 100 - Ind0Param3 - sigma;
...
void ManageClose(void)
  {
   // Stochastic (16, 2, 9), Level: 51.0
   double ind3buffer[]; CopyBuffer(indHandlers[0][3][0], MAIN_LINE, 1, 3, ind3buffer);
   double ind3val1  = ind3buffer[2];
   double ind3val2  = ind3buffer[1];
   bool   ind3long  = ind3val1 < Ind3Param3 - sigma && ind3val2 > Ind3Param3 + sigma;
   bool   ind3short = ind3val1 > 100 - Ind3Param3 + sigma && ind3val2 < 100 - Ind3Param3 - sigma;
...
//+------------------------------------------------------------------+
/*STRATEGY MARKET Exness-MT5Trial6; XAUUSDm; M5 */
/*STRATEGY CODE {"properties":{"entryLots":0.1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":100,"takeProfit":100,"useStopLoss":false,"useTakeProfit":false,"isTrailingStop":false},"openFilters":[{"name":"Stochastic","listIndexes":[2,0,0,0,0],"numValues":[2,2,1,91,0,0]},{"name":"Moving Average","listIndexes":[4,0,3,0,0],"numValues":[20,0,0,0,0,0]},{"name":"Williams' Percent Range","listIndexes":[0,0,0,0,0],"numValues":[30,-20,0,0,0,0]}],"closeFilters":[{"name":"Stochastic","listIndexes":[5,0,0,0,0],"numValues":[16,2,9,51,0,0]}]} */
